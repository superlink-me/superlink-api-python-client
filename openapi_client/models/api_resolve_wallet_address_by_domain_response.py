# coding: utf-8

"""
    Superlink

    API for Superlink  # noqa: E501

    The version of the OpenAPI document: v0.0.4-alpha.10
    Contact: support@superlink.me
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""


from __future__ import annotations
import pprint
import re  # noqa: F401
import json


from typing import List, Optional
from pydantic import BaseModel, StrictStr, conlist
from openapi_client.models.api_dns_record import ApiDNSRecord
from openapi_client.models.api_wallet_data import ApiWalletData

class ApiResolveWalletAddressByDomainResponse(BaseModel):
    """
    ApiResolveWalletAddressByDomainResponse
    """
    domain: Optional[StrictStr] = None
    records: Optional[conlist(ApiDNSRecord)] = None
    wallets: Optional[conlist(ApiWalletData)] = None
    __properties = ["domain", "records", "wallets"]

    class Config:
        """Pydantic configuration"""
        allow_population_by_field_name = True
        validate_assignment = True

    def to_str(self) -> str:
        """Returns the string representation of the model using alias"""
        return pprint.pformat(self.dict(by_alias=True))

    def to_json(self) -> str:
        """Returns the JSON representation of the model using alias"""
        return json.dumps(self.to_dict())

    @classmethod
    def from_json(cls, json_str: str) -> ApiResolveWalletAddressByDomainResponse:
        """Create an instance of ApiResolveWalletAddressByDomainResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    def to_dict(self):
        """Returns the dictionary representation of the model using alias"""
        _dict = self.dict(by_alias=True,
                          exclude={
                          },
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of each item in records (list)
        _items = []
        if self.records:
            for _item in self.records:
                if _item:
                    _items.append(_item.to_dict())
            _dict['records'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in wallets (list)
        _items = []
        if self.wallets:
            for _item in self.wallets:
                if _item:
                    _items.append(_item.to_dict())
            _dict['wallets'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> ApiResolveWalletAddressByDomainResponse:
        """Create an instance of ApiResolveWalletAddressByDomainResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ApiResolveWalletAddressByDomainResponse.parse_obj(obj)

        _obj = ApiResolveWalletAddressByDomainResponse.parse_obj({
            "domain": obj.get("domain"),
            "records": [ApiDNSRecord.from_dict(_item) for _item in obj.get("records")] if obj.get("records") is not None else None,
            "wallets": [ApiWalletData.from_dict(_item) for _item in obj.get("wallets")] if obj.get("wallets") is not None else None
        })
        return _obj

